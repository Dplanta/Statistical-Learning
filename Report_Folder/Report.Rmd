...
-	**FG_PCT**: “Field Goal Percentage”, the percentage of field goal attempts that a player scores (both 2pt and 3pt). Formula: (FGM)/(FGA);
-	**FG3_PCT**: “3 Points “Field Goal Percentage”, the percentage of 3pt field goal attempts that a player scores;
...
The boxplot reveals that the salary distribution is right-skewed, with some outliers on the higher end. This distribution was anticipated, as in basketball, as in most other sports, we observe few players excelling and commanding high salaries while the majority of players fall within a narrower performance and salary range. The histogram also highlights the right skewed distribution. It can be seen that Salary's log transformation reduces the skewness and makes the distribution of the variable closer to normal. 
...
Through the residual analysis it is noticeable that the relationship between fitted values and residuals is not exactly linear (1st graph). Additionally, in the third graph the points are not are included in a band of constant amplitude parallel to the x-axis, hence the omoschedasticity assumption can be doubted. From the fourth graph we can observe that the outliers appear to have low leverage, meaning that they don't influence strongly the parameters
...
```{r overpaid-underpaid-lm.ess, echo=FALSE, message=FALSE, warning=FALSE}
...
create_tables <- function(real_values, pred_values, df, N) {
  ...
  # format the tables
  overpaid_table <- as.data.frame(overpaid_table)
  underpaid_table <- as.data.frame(underpaid_table)
...
Russell Westbrook is in the waning phase of his career. On the expiry of his last superstar contract, no team in the league offered him a comparable salary (he earned 47 millions in 2022). Consequently, he accepted a 3.8 millions salary (veteran minimum contract) to play with Los Angeles Clippers. For sure he is no longer a player worth 47 millions, but he is not worth 3.8 millions either. Our model interprets pretty well the situation, stating that Westbrook should earn a 18.3 millions salary: not a superstar's salary, but also not minimum wage either. 

Given the presence of correlations between the predictors, the presence of multicollinearity is likely. For this reason, we decided to implement models that perform well when the variables are collinear such as Ridge regression and Lasso regression. In the next paragraphs we want to see if the performances of these models are better than that of the models seen so far.
...
We notice some similarities with the previous model: Beal, Poole, LaVine, Ayton, Garland and Porter Jr. still result in the most overpaid players.
...
At first glance we can see that Ridge regression does not classify players with very high salaries (such as Lebron James and Kevin Durant) as the most underpaid. We believe that in this regard the prediction is better than the previous model's.
...
Secondly, we implemented a lasso regression model for each group to explore the relationship between salaries and performance for each position. The objectives are:
...
```{r centers-lambda, echo=FALSE, fig.cap="Plot of the MSE with respect to $\\lambda$ in the Lasso regression model for centers \\label{fig:centers-lambda}", fig.align='center', out.width='90%'}
...
We chose the $\lambda$ value that guarantees the lower mean cross-validated error, as it can be seen in the Figure \ref{fig:centers-lambda}.

```{r centers-plot, echo=FALSE, fig.cap="Plot of the values of the parameters with respect to $\\lambda$ in the Lasso regression model for centers \\label{fig:centers-plot}", fig.align='center', out.width='90%'}
...
Once chosen the best $\lambda$, we fitted the model on all data. The Figure \ref{fig:centers-plot} shows how the coefficient estimates change increasing $\lambda$. 
...
kable(lm.las.c.tables[[1]], format="simple", caption="Three most overpaid centers according to the Lasso regression model by position")
kable(lm.las.c.tables[[2]], format="simple", caption="Three most underpaid centers according to the Lasso regression model by position")
```
...
```{r forwards-lambda, echo=FALSE, fig.cap="Plot of the MSE with respect to $\\lambda$ in the Lasso regression model for forwards \\label{fig:forwards-lambda}", fig.align='center', out.width='90%'}
...
As for centers, we started from the complete model and we chose the best $\lambda$, as it can be seen in the Figure \ref{fig:forwards-lambda}. 


```{r forwards-plot, echo=FALSE, fig.cap="Plot of the values of the parameters with respect to $\\lambda$ in the Lasso regression model for forwards \\label{fig:forwards-plot}", fig.align='center', out.width='90%'}
...
Once chosen the best $\lambda$, we fitted the model on all data. In this specific lasso regression 15 variables are reduced to zero (\ref{fig:forwards-plot}), far more than in the lasso regression for centers (9) and the general lasso regression (6).
...
kable(lm.las.f.tables[[1]], format="simple", caption="Three most overpaid forwards according to the Lasso regression model by position")
kable(lm.las.f.tables[[2]], format="simple", caption="Three most underpaid forwards according to the Lasso regression model by position")
```
...
```{r guards-lambda, echo=FALSE, fig.cap="Plot of the MSE with respect to $\\lambda$ in the Lasso regression model for guards \\label{fig:guards-lambda}", fig.align='center', out.width='90%'}
...
```{r guards-plot, echo=FALSE, fig.cap="Plot of the values of the parameters with respect to $\\lambda$ in the Lasso regression model for guards \\label{fig:guards-plot}", fig.align='center', out.width='90%'}
...
print(paste("MSE = ", format(mse.lm.las.g, scientific = TRUE)))
```
...
```{r guards-overpaid-underpaid, echo=FALSE}

### 3 most overpaid and 3 most underpaid centers table ###
lm.las.g.tables <- create_tables(y.g, lm.las.g.pred, fd_guard_nopos, 3)
kable(lm.las.g.tables[[1]], format="simple", caption="Three most overpaid guards according to the Lasso regression model by position")
kable(lm.las.g.tables[[2]], format="simple", caption="Three most underpaid guards according to the Lasso regression model by position")
```
...
```{r final-model-comparison, include=FALSE}

# idea: tabella con modello, numero dei predittori, R2 e MSE 
...
We considered only the 2023/2024 regular season: to have more complete models one would have to consider more seasons and, as mentioned at the beginning, also consider the playoffs.
