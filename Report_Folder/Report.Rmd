...
print(overpaid_tab_ex[order(overpaid_tab_ex$diff_over, decreasing = TRUE), ]   )
print(underpaid_tab_ex[order(underpaid_tab_ex$diff_under, decreasing = TRUE), ]   )


```
Here we have a comparison between real salaries and predicted ones. The two tables contain, respectively, the most overpaid players and the most underpaid players according to the model.
...
Due to the presence of correlations between the dependent variables, we decided to implement models that perform well when the variables are collinear such as Ridge regression and Lasso regression. In the next paragraphs we want to see if the performance of these models is better than that the models seen so far.
