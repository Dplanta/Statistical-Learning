...
Firstly, we perform an analysis, which can be seen in the Figure \ref{fig:variable-salary}, of the variable Salary. We considered two transformations of this variable, using logarithm and square root. 
...
The boxplot shows that the salary distribution is right skewed, with some outliers in the right side. We expected this kind of distribution, the outliers are the players earning the highest salaries. The histogram also highlights the right skewed distribution. It can be seen that Salary's log transformation reduces the skewness and makes the distribution of the variable closer to normal. After the square root transformation, the salary keeps a right skewed distribution but in a more symmetrical manner than the original variable.  
...
mse.lm.sqrt
print(paste("MSE of the complete linear model with logarithmic Salary = ", format(mse.lm.sqrt, scientific = TRUE)))
...
Applying a logarithmic transformation to the dependent variable `Salary`, the first graph showed a more linear relationship and the third graph allows to infer a more constant variance in the error terms. Nevertheless, the deterioration in performance suggests that the logarithmic transformation is not the most suitable. For this reason, we have chosen to use the square root.
The performance was better compared to the other two models: 0.7089 R-Squared, 3.607895e+13 MSE. Moreover, as can be seen in the Figure \ref{fig:square-root-transformation}, the relationship is more linear and the variance in the error terms is more constant (compared to the model without transformations). 
In both models many variables are not significant in determining the response: for this reason, to avoid a model that is unnecessary complex, we performed a variable selection. A square root transformation of the dependent variable Salary will be applied because it improves the performance of the complete model, it makes the salaries distribution closer to normal, it improves the linearity of the model and it reduces residuals eteroschedasticity.


```{r test-mse-sqrt-complete-model, include=FALSE}
...
The estimated MSE on the test set is 
...
```{r best_model_14_parameters, echo=FALSE}
...
print(paste("MSE square root of the reduced linear model with square root Salary = ",format(sqrt(mse.lm.ess), scientific=TRUE)))
...
```{r ridge-square-root-lambda, echo=FALSE, fig.cap="Plot of the MSE with respect to the value of $\\lambda$ in the Ridge regression model \\label{fig:ridge-lambda}", fig.align='center', out.width='90%'}
...
In the plot of the Figure \ref{fig:ridge-lambda} the red dotted line represents the cross-validation curve along with upper and lower standard deviation curves along the $\lambda$ sequence (error bars). We chose the value of $\lambda$ (153) that gives minimum mean cross-validated error.
...
Once fitted the model on all data with the best lambda, we evaluated the performance on the whole dataset. 

The 0.72 R-squared highlights a very good fit; also the 4.008e+13 MSE is a good result.

Considering the MSE on the test set, 4.867e+13, the Ridge regression is outperformed by the model obtained with the subset selection (4.449e+13).
...
All in all, Ridge regression has shown satisfactory performances. By making a comparison with the reduced model, R-squared slightly improves from 0.7061 to 0.7175 but it was quite expected since Ridge considers many more predictors. For what concerns the MSE calculated both on the whole dataset and the test set, the reduced model outperforms the Ridge regression. Consequently, we can say that the reduced model is better in terms of precision and parsimony. 
...
Observing the coefficients, the model contains 9 variables. Among them, AGE, PTS, MIN_G and WS that were strongly significant in the best subset selection model. 
...
Also in this case, 8 out of 10 players are the same as in ridge and the differences are really small. One of the changes is the presence of Kevin Love. As Eric Gordon, he is a veteran and he signed for a small salary with Miami Heat.
...
We now consider only the centers, 67 players. 
...
Starting from the complete model (considering again the square root transformation of Salary), we used a Lasso regression. 
...
It is interesting that, in the particular case of centers, 13 features are considered: the model is more complex than the general lasso model (9 predictors). Unexpectedly, despite the fact that blocks are typically a center play, the variable BLK is excluded from the model.
...
Regarding guards, we consider 148 observations. Here a model obtained through a subset selection performs better.
...
```{r ess-guards-analysis, echo=FALSE}
...
Summarizing, we started from a linear regression model and we applied a logarithmic transformation to the dependent variable salary. Then, in order to exclude unnecessary or redundant variable and to obtain a simpler model, we performed a variable selection. We got a simpler model (from x to y predictors) with a more than acceptable decrease in performance. However, given the probable presence of multicollinearity between the predictors, we implemented a ridge regression model and a lasso regression model. Indeed, ridge regression showed an improvement in both R-squared and MSE that was further enhanced by the lasso regression. 
Since lasso regression was the best model in terms of performance, we used it to analyse separately players with different roles. The models for centers and forwards have shown a really good fit with the data. Moreover, the variables considered changed both in number and type compared to the model fitted on all players. For what concerns guards, a very simple model emerged but with significantly worse performance.
