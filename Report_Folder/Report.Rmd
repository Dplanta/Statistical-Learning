...
With a logarithmic transformation of the dependent variable, the model shows a lower R-squared (0.6516) and a slightly higher MSE (4.70e+13). This can be seen in the Figure \ref{fig:logarithmic-transformation}.
...
The reduced model shows a 0.6457 R-squared, that indicates a quite good fit.
...
The Mean Squared Error of the reduced model is really close to the one of the complete linear model with the logarithmic transformation of the Salary, 4.77e+13 against 4.70e+13. In order to compare the two models we used the Bayesian Information Criterion (BIC) since the models have a different number of predictors. The BIC for the reduced model is than that of the full model (753.9 against 842): for this reason, according to this measure, the reduced model has a better balance of fit and parsimony.
...
Once fitted the model on all data with the best lambda, we evaluated the performance. 

The 0.69 R-squared highlights a fairly good fit; *ha senso il confronto con il modello non logaritmico?* 
Also the MSE improves: we get 4.34e+13 instead of the previous 4.77e+13.
