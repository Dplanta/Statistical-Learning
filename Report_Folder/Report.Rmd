...
Before studying the data with formal models, we got an overview through an exploratory data analysis. For the first part of our analysis we used only numeric variables, so PLAYER_NAME and POS are removed from the dataset at this stage.  
...
In order to study correlations between the variables that will be the independent ones in the models, we used the corrplot function.

```{r corrplot-independent-variables, warning=FALSE}
...
It emerges that many variables are not significative in determining the response. *For this reason, to avoid a model that is unnecessary complex, we performed a variable selection* (inserire dopo)
Attraverso l'analisi dei residui notiamo che il primo grafico non è proprio lineare e che il terzo non ha i punti racchiusi in
due fasce orizzontali (eteroschedasticità). 
perciò trasformazione logaritmica per appianare 
...
```
ancora molte variabili non sono significative, ma migliorano il primo grafico (più lineare) e il terzo.

in entrambi i modelli molte variabili non sono significative, quindi subset selection. Partiremo utilizzando il logaritmo perchè abbiamo visto che rende più normale la forma di y, la relazione diventa più lineare(primo grafico) e ci sono meno problemi di eteroschedasticità (terzo grafico)
