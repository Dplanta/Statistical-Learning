...
print(paste("MSE of the full linear model = ", format(mse.lm.mod, scientific = TRUE)))
```
...
print(paste("MSE of the full linear model with logarithmic Salary = ", format(mse.lm.log, scientific = TRUE)))
```
...
```{r parameters-selection, echo=FALSE, message=FALSE, warning=FALSE}
library(leaps)
...
print(paste("MSE of the full linear model with logarithmic Salary = ", format(test.mse.lmex, scientific = TRUE)))
```
...
lm.ess.tables <- create_tables(y, exp(lmex.final.pred), 10)
lm.ess.tables[[1]]
lm.ess.tables[[2]]
...
```
Here we compare real salaries and predicted ones. The tables contain, as before, the 10 most overpaid players and the 10 most underpaid players according to the model. 
...
At first glance we can see that ridge regression does not classify players with very high salaries (such as Lebron James and Kevin Durant) as most underpaid. We believe that in this respect the prediction is better than the previous model one.
...
```
Here we compare real salaries and the ones predicted with Lasso regression.


*MOST OVERPAID PLAYERS*

It is interesting to note that 9 out of 10 players in this table are the same as in the corresponding table for ridge. Also the difference between real salaries and predicted ones is very similar to that of the ridge. The only change is the presence of Trae Young here (he was one the most overpaid players in the best subset selection model) and Rudy Gobert in the ridge. 


*MOST UNDERPAID PLAYERS*

Also in this case, 9 out of 10 players are the same as in ridge and the differences are really similar. 
The only change is the presence of Kevin Love. As Eric Gordon, he is a veteran and he signed for a small salary with Miami Heat.


After analysing the three models, it emerges that ridge and lasso regression outperform the model obtained through the best subset selection. Additionally, the predictions for the most underpaid players seem to be more reasonable in ridge and lasso regression. 
